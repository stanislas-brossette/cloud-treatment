...
EuclidianClustersExtractionCell::EuclidianClustersExtractionCell():
	Cell("EuclidianClustersExtractionCell")
{
...
planCloudsPtr_t EuclidianClustersExtractionCell::compute(planCloudsPtr_t planCloudListPtr)
{
	cluster_tolerance_ = parameters()["cluster_tolerance"];
	...
	for(std::size_t j = 0;j<planCloudListPtr->size(); ++j)
	{
		...
		euclidian_cluster_extractor_.setInputCloud (planCloudListPtr->at(j).cloud());
			 ...
			 cluster_indices_out.begin (); it != cluster_indices_out.end (); ++it)
		{
			...
			for (std::vector<int>::const_iterator pit = it->indices.begin ();
				...
				plan_cloud_ptr_->cloud()->points.push_back
						(planCloudListPtr->at(j).cloud()->points[static_cast<unsigned long>(*pit)]);
